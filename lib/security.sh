...
_security_audit() {
    ...
    _check "SSH root login kapalı" \
        "grep -rq 'PermitRootLogin no' /etc/ssh/sshd_config.d/ 2>/dev/null"
    _check "SSH şifre auth kapalı" \
        "grep -rq 'PasswordAuthentication no' /etc/ssh/sshd_config.d/ 2>/dev/null"
    _check "SSH varsayılan port değil" \
        "! grep -rq 'Port 22$' /etc/ssh/sshd_config.d/ 2>/dev/null"
    ...
    _check "symlink koruması (sysctl)" \
        "sysctl -n fs.protected_symlinks 2>/dev/null | grep -q '1'"
    _check "ASLR aktif (randomize_va_space=2)" \
        "test \$(sysctl -n kernel.randomize_va_space 2>/dev/null) -eq 2"
    ...
    _check "Sadece localhost dinliyor" \
        "grep -q 'bind-address = 127.0.0.1' /etc/mysql/mariadb.conf.d/99-srvctl-security.cnf 2>/dev/null"
    _check "local-infile kapalı" \
        "grep -q 'local-infile = 0' /etc/mysql/mariadb.conf.d/99-srvctl-security.cnf 2>/dev/null"
    _check "symbolic-links kapalı" \
        "grep -q 'symbolic-links = 0' /etc/mysql/mariadb.conf.d/99-srvctl-security.cnf 2>/dev/null"
    ...
    _check "expose_php = Off" \
        "grep -q 'expose_php = Off' '${php_ini}' 2>/dev/null"
    _check "display_errors = Off" \
        "grep -q 'display_errors = Off' '${php_ini}' 2>/dev/null"
    _check "allow_url_fopen = Off" \
        "grep -q 'allow_url_fopen = Off' '${php_ini}' 2>/dev/null"
    _check "allow_url_include = Off" \
        "grep -q 'allow_url_include = Off' '${php_ini}' 2>/dev/null"
    _check "disable_functions tanımlı" \
        "grep -q 'disable_functions' '${php_ini}' 2>/dev/null"
    _check "Varsayılan www pool kapalı" \
        "! test -f /etc/php/${DEFAULT_PHP_VERSION}/fpm/pool.d/www.conf"
